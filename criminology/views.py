...
from django.db.models import Max,Q, F, Subquery, OuterRef,Count, Sum, Case, When, Value, IntegerField
import random as rd
...
def submit_table_of_specification(request, group_id):
    if request.method == 'POST':
        for key, value in request.POST.items():
            if key.startswith(('pwd_', 'remembering_', 'understanding_', 'applying_', 'analyzing_', 'evaluating_', 'creating_')):
                try:
                    subtopic_id = key.split('_')[1]
                    field = key.split('_')[0]

                    entry = TableOfSpecification.objects.get(group_id=group_id, subtopic=subtopic_id)

                    academic_year_value = entry.academic_year
                    row_id_value = entry.row_id
                    group_id_value = entry.group_id
                    subject_value = entry.subject
                    topic_value = entry.topic
                    subtopic_value = entry.subtopic
                    pwd_value = entry.pwd if entry.pwd is not None else 0.0
                    understanding_value = entry.understanding
                    remembering_value = entry.remembering
                    analyzing_value = entry.analyzing
                    creating_value = entry.creating
                    evaluating_value = entry.evaluating
                    applying_value = entry.applying

                    entry.delete()

                    if field == 'pwd':
                        pwd_value = float(value) if value else 0.0
                    else:
                        locals()[f'{field}_value'] = int(value) if value.isdigit() else 0

                    new_entry = TableOfSpecification(
                        ...
                        pwd=pwd_value,
                        understanding=int(request.POST.get(f'understanding_{subtopic_id}', understanding_value)),
                        remembering=int(request.POST.get(f'remembering_{subtopic_id}', remembering_value)),
                        analyzing=int(request.POST.get(f'analyzing_{subtopic_id}', analyzing_value)),
                        creating=int(request.POST.get(f'creating_{subtopic_id}', creating_value)),
                        evaluating=int(request.POST.get(f'evaluating_{subtopic_id}', evaluating_value)),
                        applying=int(request.POST.get(f'applying_{subtopic_id}', applying_value))
                    )
                    new_entry.save()

                except TableOfSpecification.DoesNotExist:
                    print(f"No entry found for group_id {group_id} and subtopic_id {subtopic_id}")
                except ValueError as e:
                    print(f"ValueError for subtopic_id {subtopic_id} and field {field}: {e}")
    ...
    if request.method == 'POST':
        ...
        for topic in topics:
            ...
            for subtopic in subtopics:
                ...
                sub_pwd = int(request.POST.get(f'subtopic_pwd_{subtopic.id}', 0))
                ...
                tos_percentage = PercentageWeightPerTos(
                group_id = group_id,
                row_id=row_id,
                pwd = sub_pwd,
                )
...
def assessment_create(request):
    ...
    percentage_w_assess = request.POST.get('percentage_w_assessment')

    if not subject_id or not topic_id or not percentage_w_assess:
        return render(request, 'assets/masterfileassessment_create.html', {
            'error': 'Please select both Subject, Topic, and Percentage Weight. ',
            'subject': subject,
        })
    ...
    if request.method == "POST":
        ...
        assessment_percentage = PercentageWeightPerAssessment(
        assessment_id=assessment_id,
        percentage_w_assess = percentage_w_assess,
        )
        assessment_percentage.save()
...
def assessment_update(request, assessment_id):
    ...
    if request.method == 'POST':
        ...
        if per:
            try:
                percentage.percentage_w_assess = int(per)
                percentage.save()
            except ValueError:
                messages.error(request, "Invalid value for percentage weight.")
        ...
        try:
            for assessment in assessments_to_update:
                ...
                values = [int(remembering), int(understanding), int(applying), int(analyzing), int(evaluating), int(creating)]
                if any(val < 0 for val in values):
                    messages.error(request, "All values must be non-negative.")
                    return redirect(request.path)

                assessment.remembering = int(remembering)
                assessment.understanding = int(understanding)
                assessment.applying = int(applying)
                assessment.analyzing = int(analyzing)
                assessment.evaluating = int(evaluating)
                assessment.creating = int(creating)
                assessment.save()
...
def students_all_stats_assessment(request, studentid):
    try:
        ...
        for stat in stud_stats:
            ...
            if pwa:
                exam_to_percentage[stat.exam_id] = pwa.percentage_w_assess
            else:
        ...
        for stat in stud_stats:
            ...
            subject_totals[subject_code]['total_percentage_w_assess'] += exam_to_percentage.get(stat.exam_id) or 0
