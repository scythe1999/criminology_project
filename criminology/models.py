...
class TableOfSpecification(models.Model):
    ...
    pwd = models.FloatField(null=True, blank=True)
...
class PercentageWeightPerAssessment(models.Model):
    status = models.IntegerField(default=0)
    percentage_w_assess = models.IntegerField(null=True, blank=True)
...
class StudentStatsTos(models.Model):
    ...
    def percentage_w_per_row(self):
        try:
            tos_rows = AnswerKeyTableOfSpecification.objects.filter(tos_exam_id=self.tos_exam.tos_exam_id, row_id=self.row_id)
            percentage_weights = PercentageWeightPerTos.objects.filter(row_id__in=[tos.row_id for tos in tos_rows])
            if percentage_weights.exists():
                return sum([pw.pwd for pw in percentage_weights]) / len(percentage_weights)
            return None
    ...
    def contribution(self):
        ...
        if percentage_weight is None:
            return 0
        return round((self.correct_count / self.question_count) * percentage_weight, 2)
    ...
    def total_percentage_w_per_row(self):
        student_stats = StudentStatsTos.objects.filter(student=self.student)
        total = sum(stat.percentage_w_per_row for stat in student_stats if stat.percentage_w_per_row is not None)
        return round(total, 2)
...
class StudentStatsAssessment(models.Model):
    ...
    def contribution(self):
        ...
        if pwa and self.total_questions_taken != 0:
            contribution = (self.total_questions_score / self.total_questions_taken) * pwa.percentage_w_assess
    ...
    def score_percentage(self):
        ...
        total_percentage_w_assess = sum(
            PercentageWeightPerAssessment.objects.filter(assessment_id=stat.exam_id, status=0).first().percentage_w_assess
            for stat in stud_stats if PercentageWeightPerAssessment.objects.filter(assessment_id=stat.exam_id, status=0).exists()
        )
